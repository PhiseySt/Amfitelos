...
Comment:= 'Ход черных.Найдите оптимальный вариант. Сделайте ничью.' ;
Comment2:='Поля соответствия.';
Comment3:='Эффективным приемом в пешечном окончании может стать оппозиция.';
Uroven:=1;
end;
if (partNum=2) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Минированные поля.';
Comment3:='Белые могут выиграть засчет цугцванга.';
Uroven:=1;
end;
if (partNum=3) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Пешечные гонки.';
Comment3:='Черная пешка h6 пойдет вперед, стоит обратить внимание что поля h1 и a8 находятся на одной диагонали.';
Uroven:=1;
end;
if (partNum=4) then
begin
...
Comment:= 'Ход черных.Найдите оптимальный вариант.' ;
Comment2:='Прорыв.';
Comment3:='Позиция черных кажется трудной, поскольку король противника хозяйничает на ферзевом фланге. Однако возможность прорыва меняет оценку.';
Uroven:=1;
end;
if (partNum=5) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Зигзаг.';
Comment3:='Белые выигрывают засчет того, что их ферзь встает с шахом.';
Uroven:=1;
end;
if (partNum=6) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Отдаленная проходная.';
Comment3:='Белые выигрывают выманивая черного короля.';
Uroven:=1;
end;
if (partNum=7) then
begin
...
Comment:= 'Ход черных.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Подрыв.';
Comment3:='Белый король прикован к черным пешкам. Организуем еще одну проходную по линии a.';
Uroven:=1;
end; 
if (partNum=8) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Две связанные проходные.';
Comment3:='На доске типичная ситуация с двумя связанными проходными.Кажется, что ничья неизбежна, поскольку белый король прикован к квадрату проходной пешки с4.';
Comment4:='На самом деле в таких случаях прой удается покинуть квадрат, что бы помочь своим пешкам пройти в ферзи или заматовать противника.';
Uroven:=1;
end; 
if (partNum=9) then
begin
...
Comment:= 'Ход белых.Сделайте ничью.' ;
Comment2:='Пешечный эндшпиль.Игра на пат.';
Comment3:='Белые неизбежно теряют пешку f5, но их спасает возможность запатовать короля.';
Uroven:=1;
end; 
if (partNum=10) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Полупат.';
Comment3:='Белые патуют черного короля и черные вынужденно теряют пешки.';
Uroven:=1;
end; 
if (partNum=11) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Правило Стейница.';
Comment3:='Белые выигрывают засчет выигрыша темпа.';
Uroven:=1;
end;
if (partNum=12) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Пешки g и h против пешки h.';
Comment3:='Белые выигрывают засчет выигрыша темпа.';
Uroven:=1;
end; 
if (partNum=13) then
...
Comment:= 'Ход черных.Найдите оптимальный вариант.' ;
Comment2:='Пешечный эндшпиль.Запасные темпы у обеих сторон.';
Comment3:='Черные выигрывают засчет выигрыша темпа.';
Uroven:=1;
end; 
if (partNum=14) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.' ;
Comment2:='Конь против пешек.Король в углу.';
Comment3:='Белые выигрывают давая мат королю в углу.';
Uroven:=1;
end; 
if (partNum=15) then
begin
...
Comment:= 'Ход белых.Сделайте ничью.' ;
Comment2:='Конь против пешек.Конь против ладейной пешки.';
Comment3:='Коневые вилки - важнейший прием в коневых окончаниях.';
Uroven:=1;
end;
if (partNum=16) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Конь против пешек.Защита пешек конем.';
Comment3:='Белые должны препятствовать прорыву f7-f6.';
Uroven:=1;
end;
if (partNum=17) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Коневые окончания.Отвлекающая жертва коня.';
Comment3:='Отвлекающая жертва коня - прием, почти неизменно используемый в подобных окончаниях.';
Uroven:=1;
end; 
if (partNum=18) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.' ;
Comment2:='Слон против пешек.Слон и ладейная пешка.';
Comment3:='Если слон не владеет полем превращения ладейной пешки, то слабейшей стороне достаточно прийти королем в угол доски.';
Uroven:=1;
end; 
if (partNum=20) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.' ;
Comment2:='Слон против пешек.Принцип одной диагонали.';
Comment3:='Слон выполняет всю работу  по одной диагонали, и это позволяет без труда достичь ничьей.';
Uroven:=1;
end; 
if (partNum=21) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.' ;
Comment2:='Разноцветные слоны.Слон и две связанные пешки против слона.';
Comment3:='Как только пешки блокируются на полях цвета своего слона, ничейный исход становится очевидным.';
Uroven:=1;
end; 
if (partNum=22) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант.' ;
Comment2:='Разноцветные слоны.Король блокирует проходную пешку.';
Comment3:='Король блокирует проходную пешку противника, слон защищает свои пешки. Попытки разрушения обороны всегда связаны с образованием проходной пешки.';
Uroven:=1;
end; 
if (partNum=23) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Одноцветные слоны. Слон и пешка против слона.';
Comment3:='Даже при "правильном" расположении короля достичь ничьей не удается, если одна из диагоналей, по которой слон задерживает пешку, оказывается слишком короткой.';
Uroven:=1;
end;
if (partNum=24) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Одноцветные слоны. Переход к позициям с одной пешкой.';
Comment3:='Даже при "правильном" расположении короля достичь ничьей не удается, если одна из диагоналей, по которой слон задерживает пешку, оказывается слишком короткой.';
Uroven:=1;
end;
if (partNum=25) then
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Одноцветные слоны. Перекрытие.';
Comment3:='Перекрытие основной инструмент, с помощью которого сильнейшая сторона обеспечивает проведение пешки в ферзи. Иногда перекрытие осуществляется с помощью пешек.';
Uroven:=1;
end; 
if (partNum=26) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Одноцветные слоны. Цугцванг.';
Comment3:='Надежды слабейшей стороны удержать оборону при плохом слоне зачастую рушатся из-за цугцванга.';
Uroven:=1;
end; 
if (partNum=27) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.' ;
Comment2:='Одноцветные слоны.Барьер.';
Comment3:='Надо контролировать слоном поля одного цвета, а пешками другого. Тогда на пути у неприятельского короля возникнет барьер.';
Uroven:=1;
end; 
if (partNum=28) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.' ;
Comment2:='Слон против коня.Конь и пешка против слона.';
Comment3:='Если слон не справляется в одиночку, то исход борьбы зависит от положения короля слабейшей стороны: способен ли он воспрепяствовать перекрытию или вытеснению.';
Uroven:=1;
end; 
if (partNum=29) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант' ;
Comment2:='Слон против коня. Отрезание коня.';
Comment3:='Если конь расположен на краю доски, слон способен лишить его ходов.';
Uroven:=1;
end; 
if (partNum=30) then
begin
...
Comment:= 'Ход черных.Найдите оптимальный вариант' ;
Comment2:='Слон против коня.Барьер.';
Comment3:='Надо контролировать слоном поля одного цвета, а  пешками другого. Тогда на пути у неприятельского короля возникнет барьер.';
Uroven:=1;
end; 
if (partNum=31) then
...
Comment:= 'Ход черных.Найдите оптимальный вариант' ;
Comment2:='Слон против коня.Коневые вилки.';
Comment3:='Конь создает угрозу вилки отвлекая короля от отдаленной проходной.';
Uroven:=1;
end;
if (partNum=32) then
begin
...
Comment:= 'Ход черных.Сделайте ничью' ;
Comment2:='Ладья против пешек.Игра на пат.';
Comment3:='Черные патуют своего короля.';
Uroven:=1;
end; 
if (partNum=33) then
begin
...
Moves := 'b2-a3';
Comment:= 'Ход черных.Сделайте ничью' ;
Comment2:='Ладья против пешек.Отталкивание плечом.';
Comment3:='Не подпуская короля к пешке, черные добиваются ничьей.';
Uroven:=1;
end; 
if (partNum=34) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант' ;
Comment2:='Ладья против связанных пешек. Смена лидера.';
Comment3:='В ферзи надо вести пешку b. Потому, что ее продвижению пока не препятствует неприятельская ладья. ';
Uroven:=1;
end; 
if (partNum=35) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант' ;
Comment2:='Ладейные окончания.Ладья и пешка против ладьи';
Comment3:='Черная ладья вынужденно должна держаться на 8-й горизонтали.Белые легко побеждают переведя ладью на h7 ';
Uroven:=1;
end;
if (partNum=36) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант' ;
Comment2:='Ладейные окончания.Ладья и пешка против ладьи. Зонтик';
Comment3:='Собственная пешка мешает белым спастись, дав шах ладьей. ';
Uroven:=1;
end; 
if (partNum=37) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант' ;
Comment2:='Ладейные окончания.Ладья и пешка против ладьи. ';
Comment3:='Сначала черные максимально активизировают короля (идеальное место для него через одну клетку по диагонали), а затем защищают пешку ладьей.  . ';
Uroven:=1;
end; 
if (partNum=38) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант' ;
Comment2:='Ладейные окончания.Ладья и ладейная пешка против ладьи. ';
Comment3:='Белые выиигрывают, поскольку король не поспевает на с7.';
Uroven:=1;
end; 
if (partNum=39) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант' ;
Comment2:='Ладейные окончания.Ладья и ладейная пешка против ладьи. ';
Comment3:='Белые выиигрывают, заманивая короля на f7.';
Uroven:=1;
end; 
if (partNum=40) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.' ;
Comment2:='Ладейные окончания.Ладья и ладейная пешка против ладьи. Позиция Ванчуры ';
Comment3:='Ладья атакует пешку, что бы не выпустить неприятельскую ладью с поля а8.';
Uroven:=1;
end; 
if (partNum=41) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Ладейные окончания.Ладья и две пешки против ладьи. ';
Comment3:='Пешки двигаются в ферзи лишь при помощи ладьи.';
Uroven:=1;
end; 
if (partNum=42) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.' ;
Comment2:='Ладейные окончания.Ладья и две пешки против ладьи. ';
Comment3:='Белые добиваются ничьи с помощью пата.'; 
Uroven:=1;
end; 
if (partNum=43) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.' ;
Comment2:='Ладейные окончания.Далеко продвинутая проходная пешка. Идея Ласкера ';
Comment3:='Белые включают механизм постепенного оттеснения неприятельского короля аж до 2-й горизонтали .'; 
Uroven:=1; 
end; 
if (partNum=44) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Ладейные окончания.Разрозненные пешки. ';
Comment3:='Слабейшая сторона должна стремиться к активной контригре.Если ладье приходится лишь защищать собственную пешку или оберегать короля от шахов, то спастись удается редко .'; 
Uroven:=1; 
end; 
if (partNum=45) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.' ;
Comment2:='Ладейные окончания.Отрезание короля. ';
Comment3:='Важным приемом разыгрывания ладейных окончаний является отрезание короля от стратегически важных участков доски.'; 
Uroven:=1; 
end;
if (partNum=46) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.' ;
Comment2:='Ладья против коня.Ладья с пешкой против коня с пешкой.';
Comment3:='Конь на е4 препятствует вторжению короля через королевский фланг, но окажется под ударом после вторжения короля на е5.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.' ;
Comment2:='Ладья против коня.Конь сильнее ладьи.';
Comment3:='Порой ладье бывает нелегко справиться с далеко продвинутой пешкой, поддерживаемой конем. В таких ситуациях сказываются тактические свойства коня: вилки, отрезание и т.д.';
Uroven:=1; 
end; 
if (partNum=49) then
begin
...
Comment:= 'Ход белых.Сделайте ничью.' ;
Comment2:='Ладья против слона.Безопасный угол.';
Comment3:='Белые спасаются используя прием "перекрытие".'; 
Uroven:=1; 
end; 
if (partNum=50) then
begin
...
Moves := 'h7-h5';
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладья против слона.Две пешки против двух на одном фланге.';
Comment3:='Черные выстраивают барьер.'; 
Uroven:=1; 
end; 
if (partNum=51) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Ферзевые окончания.Выигрывающие тактические приемы.';
Comment3:='Ферзь - самая сильная фигура, поэтому в ферзевых окончаниях чаще всего чем в других встречается игра на мат. Из других тактических средств отметим выигрыш ферзя и размен ферзей.';
Uroven:=1; 
end; 
if (partNum=52) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Ферзевые окончания.Выигрывающие тактические приемы.';
Comment3:='Ферзь - самая сильная фигура, поэтому в ферзевых окончаниях чаще всего чем в других встречается игра на мат. Из других тактических средств отметим выигрышь ферзя и размен ферзей.'; 
Uroven:=1; 
end; 
if (partNum=53) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.';
Comment2:='Ферзевые окончания.Тактика при защите.';
Comment3:='Основные тактические приемы, позволяющие порой спасти трудный ферзевый эндшпиль - пат и вечный шах.'; 
Uroven:=1; 
end; 
if (partNum=54) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.';
Comment2:='Ферзевые против ладьи.Одинокая ладья.';
Comment3:='Черному королю не удается укрыться от шахов ввиду пата или потери ферзя.'; 
Uroven:=1; 
end;
if (partNum=55) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.';
Comment2:='Матование слоном и конем.';
Comment3:='Обратите внимание:слон и конь создают барьер, запирающий короля в углу.'; 
Uroven:=1; 
end;
if (partNum=56) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Ладья против двух легких фигур.';
Comment3:='Ладья особенно опасна, когда она нападает на пешки, которые не может защитить король и неудобно защищать фигурами, а также если она поддерживает отдаленную проходную.'; 
Uroven:=1; 
end;
if (partNum=57) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Пешечный эндшпиль.Идея Рети.';
Comment3:='Король догоняет проходную пешку противника, находясь вне ее квадрата.Недостающие темпы выигрываются благодаря встречным угрозам.'; 
Uroven:=1; 
end;
if (partNum=58) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Погоня за двумя зайцами.';
Comment3:='Преследуя коня, белые на самом деле стремятся приблиизить короля к пешкам.'; 
Uroven:=1; 
end;
if (partNum=59) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Перекрытие.';
Comment3:='Черные перекрывают линию с, что бы обеспечить своей пешке проход в ферзи.'; 
Uroven:=1; 
end;
if (partNum=60) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Игра на пат.';
Comment3:='Белые патуют короля пешками.'; 
Uroven:=1; 
end;
if (partNum=61) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Игра на пат.';
Comment3:='Белые жертвуют пешку и спасаются благодаря пату.'; 
Uroven:=1; 
end;
if (partNum=63) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Игра на мат.';
Comment3:='Белые матуют черного короля не обращая внимания на черную пешку а.'; 
Uroven:=1; 
end;
if (partNum=64) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Игра на мат.';
Comment3:='Белые жертвуют слона что бы предотвратить ход f7-f5.'; 
Uroven:=1; 
end;
if (partNum=65) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечный эндшпиль.Треугольник.';
Comment3:='Напрашивается план атаки пешки d5. Но его надо проводить  аккуратно, считаясь с контр игрой противника.'; 
Uroven:=1; 
end;
if (partNum=66) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Пешечный эндшпиль.Король против проходных пешек.';
Comment3:='Белый король находится в квадрате черной пешки, но догнать ее ему мешает неприятельский король.'; 
Uroven:=1; 
end;
if (partNum=67) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечный эндшпиль.Король против проходных пешек.';
Comment3:='Надо описав королем треугольник, передать очередь хода противнику. Это позволит белым продвинуть пешки на один шаг вперед.'; 
Uroven:=1; 
end;
if (partNum=68) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечный эндшпиль.Пешечные гонки.';
Comment3:='Белые добьются успеха только если смогут превратить пешку в ферзя с шахом.'; 
Uroven:=1; 
end;
if (partNum=69) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечный эндшпиль.Прорыв.';
Comment3:='Белые выигрывают засчет прорыва. Но прежде чем его осуществить, ставят короля на поле где он не попадает под шах.'; 
Uroven:=1; 
end;
if (partNum=70) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Пешечный эндшпиль.Игра на пат.';
Comment3:='Белые спасаются патуя своего короля пешками.'; 
Uroven:=1; 
end;
if (partNum=71) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечный эндшпиль.Запасные темпы.';
Comment3:='Цугцванг - основное оружие в пешечных окончаниях.Используйте любую возможность, чтобы приобрести запасные темпы.'; 
Uroven:=1; 
end;
if (partNum=72) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Конь против пешки.Король в углу.';
Comment3:='Конь дает мат королю в углу.'; 
Uroven:=1; 
end;
if (partNum=73) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Конь против пешки.Конь против ладейной пешки.';
Comment3:='Конь не успевает на h2 - он может предотвратить появление ферзя на поле h1. Значит, надо построить барьер.'; 
Uroven:=1; 
end;
if (partNum=74) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Конь против пешки.Защита пешки конем.';
Comment3:='Белые строют барьер, на время подхода их короля.'; 
Uroven:=1; 
end;
if (partNum=75) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Разноцветные слоны.Разноцветные пешки.';
Comment3:='Белые прорываются королем в тыл противника.'; 
Uroven:=1; 
end;
if (partNum=78) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Конь против слона.';
Comment3:='Белые перекрывают неприятельского слона.'; 
Uroven:=1; 
end;
if (partNum=79) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Ладья против пешек.';
Comment3:='Белые с темпом ставят ладью позади пешки и, выяснив положение неприятельского короля, направляют своего в обход.'; 
Uroven:=1; 
end;
if (partNum=80) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Конь против слона.';
Comment3:='Белые арестовывают слона противника.'; 
Uroven:=1; 
end;
if (partNum=81) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Ладейные окончания.';
Comment3:='Белые тормозят королем неприятельские пешки.'; 
Uroven:=1; 
end;
if (partNum=82) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.';
Comment2:='Ладейные окончания.';
Comment3:='Черные создают неприятельскому королю угрозу мата.'; 
Uroven:=1; 
end;
if (partNum=83) then
begin
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Ладейные против коня.';
Comment3:='Белые запирают и выигрывают черного коня.'; 
Uroven:=1; 
end;
if (partNum=84) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Ферзевые окончания.';
Comment3:='Черные выигрывают после размена ферзей и выиграного темпа.'; 
Uroven:=1; 
end;
if (partNum=85) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Отдаленная проходная.';
Comment3:='План белых: надвинуть пешки ферзевого фланга как можно дальше, разменять проходную h на пешку е и прорваться королем к пешкам противника.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Отдаленная проходная.';
Comment3:='Белые отвлекают отдаленной проходной неприятельского короля от пешек и выигрывают их.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Пешечные окончания.Игра на пат.';
Comment3:='Черные вынуждены защищать свою пешку d7, и белым удается замуровать своего короля.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Отталкивание плечом.';
Comment3:='Белые оттесняют неприятельского короля и выигрывают пешку g7.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Прорыв.';
Comment3:='Пешечный прорыв обычно связан с жертвами пешек с целью освободить дорогу пешке.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Пространство.';
Comment3:='Белые выигрывают засчет запасного темпа.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Поля соответствия.';
Comment3:='Для выигрыша необходимо передать очередь хода.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Треугольник.';
Comment3:='С помощью треугольника черные передают очередь хода белым и выигрывают.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Белые матуют неприятельского короля.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Белые жертвуют коня и проводят пешку в ферзи.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Белые применяют прием отталкивание плечом.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Белые жертвуют коня и проводят пешку.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.Барьер.';
Comment3:='Белые ставят барьер.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Для выигрыша надо передать очередь хода черным.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Коневые окончания. Прорыв';
Comment3:='В коневых окончаниях прорыву способсвует "неповоротливый" конь.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Совместными усилиями всех фигур белые дают мат.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Белые жертвуют коня и делают прорыв.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Белые догоняют неприятельскую пешку создавай встречные угрозы.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Коневые окончания.';
Comment3:='Белые жертвуют коня и патуют своего короля.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Разноцветные слоны.';
Comment3:='Пешки с5 и f6 слабы, слоном их не защитить.Задача белых - напасть на них .'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Разноцветные слоны.';
Comment3:='Белые матуют черного короля в углу .'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Одноцветные слоны.';
Comment3:='Благодаря удаленности черного короля от пешки с белые добиваются успеха .'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Одноцветные слоны.';
Comment3:='Черных губит то, что диагональ a7-b8 слишком коротка .'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Белые ставят неприятельского коня в ловушку.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Белые отрезают черного коня от пешки а.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Белые создают цугцванг.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Черные благодаря слону проводят свои пешки удерживая неприятельские.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Вместе конь и король могут выигрывать темпы.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Белые ловят неприятельского слона.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Оппозиция.';
Comment3:='Оппозицией называется противостояние королей на одной линии через одно поле (ближняя) три или пять полей (дальняя).'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Ключевые поля.';
Comment3:='При пешки на 5-й горизонтали ключевыми полями также являются поля 6-й горизонтали: а6,b6,c6.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Ключевые поля.';
Comment3:='Белые занимают оппозицию и завоевывают ключевые поля.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Ключевые поля.';
Comment3:='Черные занимают оппозицию и завоевывают ключевые поля.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Оппозиция.';
Comment3:='Белые превращают дальнюю оппозицию в ближнюю.'; 
Uroven:=1; 
end;
...
Moves := 'd4-d5';
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Король против проходных пешек.';
Comment3:='Случается, что король, хоть и находится в квадрате, все же не способен вовремя остановить прходную - мешают собственные пешки.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Сделайте ничью.';
Comment2:='Пешечные окончания. Отталкивание плечом.';
Comment3:='Белые препятствуют приему оттлакивание плечом.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Расширение плацдарма.';
Comment3:='Планы черных связаны с прокладкой королю дороги на ферзевый фланг посредством пешечных разменов.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Прорыв.';
Comment3:='Белые осуществляют прорыв.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Отдаленная проходная.';
Comment3:='Черные образуют проходную пешку, которая отвлекает белого короля.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Ферзь против пешек.';
Comment3:='Ферзь давая шах сближается с неприятельским королем и загоняет его на поле d1.В результате белый король получает темп для приближения.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Пешечные окончания. Отдаленная проходная.';
Comment3:='Отдаленная проходная отвлекает короля белых на ферзевый фланг.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых.Сделайте ничью.';
Comment2:='Пешечные окончания. Пара ладейных пешек при лишней пешке на другом фланге.';
Comment3:='Черные грозят сыграть h4, забирая затем пешку a3 и выигрывая, поскольку их пешка на другом фланге пересекла середину доски.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых.Сделайте ничью.';
Comment2:='Пешечные окончания.Пара ладейных пешек при лишней пешки на другом флпнге.';
Comment3:='Белые занимают оппозицию.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых.Найдите оптиальный вариант.';
Comment2:='Пешечные окончания.Защищенная проходная.';
Comment3:='Белые продвигают свою проходную пешку, меняя ее на пешку противника.'; 
Uroven:=1; 
end;
if (partNum=133) then
begin
...
Comment:= 'Ход черных.Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Подрыв.';
Comment3:='План выигрыша связан с подрывом f7-f6.'; 
Uroven:=1;
end;
if (partNum=134) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Подрыв.';
Comment3:='У белых есть возможность разменять пару пешек на королевском фланге, образуя проходную h.'; 
Uroven:=1; 
end;
if (partNum=135) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Две связанные проходные.';
Comment3:='Белые матуют неприятельского короля.'; 
Uroven:=1; 
end;
if (partNum=136) then
...
Comment:= 'Ход белых.Сделайте ничью.';
Comment2:='Пешечные окончания.Запасные темпы.';
Comment3:='Белые используют запасные темпы, чтобы не выпустить короля противника с края доски.'; 
Uroven:=1; 
end;
if (partNum=137) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Запасные темпы.';
Comment3:='Белые занимают оппозицию с помощью запасных темпов .'; 
Uroven:=1; 
end;
if (partNum=138) then
begin
...
Comment:= 'Ход черных.Найдите оптимальный вариант.';
Comment2:='Пешечные окончания.Пешки g и h  против пешки h.';
Comment3:='Белый король попадает "не на то" поле защищая свою пешку.'; 
Uroven:=1; 
end;
if (partNum=139) then
begin
...
Comment:= 'Ход белых.Сделайте ничью.';
Comment2:='Пешечные окончания.Пешки f и h  против пешки h.';
Comment3:='Белые ставят черных в цугцванг вынуждая двинуть пешку h7, и позиция становится ничейной.'; 
Uroven:=1; 
end;
if (partNum=140) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Слон против пешек. Слон и ладейная пешка.';
Comment3:='Черные жертвуют слона, удерживая королем пешку h.'; 
Uroven:=1; 
end;
if (partNum=142) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Слон против пешек. Слон и ладейная пешка.';
Comment3:='Черные удерживают королем пешку h.'; 
Uroven:=1; 
end;
if (partNum=143) then
begin
...
Comment:= 'Ход белых.Сделайте ничью.';
Comment2:='Слон против пешек.';
Comment3:='Белые ставят крепость.'; 
Uroven:=1; 
end;
if (partNum=145) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Слон против пешек.';
Comment3:='Черные жертвуют пешку освобождая королю поле b7.'; 
Uroven:=1; 
end;
if (partNum=146) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Разноцветные слоны.Слон и две связанные пешки против слона';
Comment3:='При разноцветных слонах черные блокируют пешки белых на полях цвета своего слона.'; 
Uroven:=1; 
end;
if (partNum=147) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Разноцветные слоны.Разрозненные проходные пешки';
Comment3:='Король сильной стороны для достижения победы идет к пешке, с которой борется слон, для проведения ее в ферзи.'; 
Uroven:=1; 
end;
if (partNum=148) then
begin
...
Comment:= 'Ход белых.Сделайте ничью.';
Comment2:='Разноцветные слоны.Слон и две связанные пешки против слона';
Comment3:='Белым необходимо избавиться от пешки b5, которая перекрывает слону диагональ a6-f1.'; 
Uroven:=1; 
end;
if (partNum=149) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Разноцветные слоны.Разрозненные проходные пешки';
Comment3:='Черные не дают белой пешки встать на d7.'; 
Uroven:=1; 
end;
if (partNum=150) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Разноцветные слоны.Разрозненные проходные пешки';
Comment3:='На всей диагонали b8-h2 слону доступны лишь поля: b8 и c7, иначе король белых с темпом нападет на них.'; 
Uroven:=1; 
end;
if (partNum=151) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Одноцветные слоны.Перекрытие.';
Comment3:='Белые перекрывают черного слона.'; 
Uroven:=1; 
end;
if (partNum=152) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='К цели приводит жертва коня с последующим прорывом пешки в ферзи.'; 
Uroven:=1; 
end;
if (partNum=153) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Коневые окончания.';
Comment3:='Белые не подпускают черного короля к пешке a.'; 
Uroven:=1; 
end;
if (partNum=154) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Слон против разрозненных пешек.';
Comment3:='Белые применяют прием "штаны".'; 
Uroven:=1; 
end;
if (partNum=155) then
begin
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Слон против пешек.';
Comment3:='Черные жертвуют слона переходя в выигранное пешечное окончание.'; 
Uroven:=1; 
end;
if (partNum=156) then
begin
...
Comment:= 'Ход черных. Сделайте ничью.';
Comment2:='Слон против пешек.';
Comment3:='Черные жертвуют слона патуя своего короля.';
Uroven:=1; 
end;
if (partNum=157) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Слон против пешек.';
Comment3:='Белые ставят противника в цугцванг.';
Uroven:=1; 
end;
if (partNum=158) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Одноцветные слоны.';
Comment3:='Белые выиграют, если продвинут пешку до b6 и проникнут королем на а7.'; 
Uroven:=1; 
end;
if (partNum=159) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Одноцветные слоны.';
Comment3:='Белым нельзя дать продвинуться пешке d6 и при этом надо перекрыть диагональ черному слону.';
Uroven:=1; 
end;
if (partNum=160) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Белые отдают очередь хода сопернику.';
Uroven:=1; 
end;
if (partNum=161) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Белый слон отрезает черного коня.';
Uroven:=1;
end;
if (partNum=163) then
begin
...
Comment:= 'Ход белых.Сделайте ничью.';
Comment2:='Слон против коня.';
Comment3:='Белые жертвуют коня за пешку и не дают пройти неприятельской пешке h в ферзи.';
Uroven:=1;
end;
if (partNum=165) then
begin
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Напрашивается отдать слона за коня, с тем чтобы король белых удачно расположился в центре.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Белым надо активизировать короля для атаки пешки f3, конь переводится на d2 также для атаки пешки.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Не хотелось бы чтобы белые активизировали своего короля, для этого подкрепим пешку f3 пешечной цепью.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='План черных предельно прост, Им надо установить короля на e5, разменять пару ладей на поле a4, после чего пешка e4 обречена.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Черные проникают королем в лагерь белых и устраивают нападение на пешки.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Черные грозят активизировать короля по маршруту d7-e6-d5-e4. Необходимо воспрепятствовать им. Для это можно предложить размен коней Nb4 с освобождением поля d3. (или возможно 26.c4)'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Черным надо активизировать короля по маршруту d7-e6-d5-e4.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля';
Comment3:='Перед нападением на пешку f7 необходимо дать промежкточный шах и откинуть активного короля черных назад.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Необходимо оттолкнуть плечем короля соперника.'; 
Uroven:=1; 
end;
...
Moves := 'g6-g5'; 
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Ладейный эндшпиль.';
Comment3:='Белые грозят образовать вторую проходную на королевском фланге. Технично заблокировать ее.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Королю придется догонять пешку g, поэтому невыгодно сейчас ставить ферзя и начинать старт с поля a8. Лучше стартовать с a7.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Для проведения пешки в ферзи необходимо активизировать короля и применить отталкивание плечом.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='При борьбе крайней пешки против ладьи надо не забывать про короля и пользоваться патовыми идеями.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Ограничить подвижность короля соперника может не только король, но и ладья.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Спасение черным следует искать в прорыве королем к проходной пешке, так как их король отрезан на длинной строне.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Спасение белым следует искать в прорыве королем к проходной пешке, так как их король отрезан на длинной строне.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Активность короля.';
Comment3:='Пешка e6 может быть уничтожена в любой момент, но спешить с этим не следует. Ладья удачно расположена на f6 и не дает активизироваться королю белых. Первоочередная задача белых подтянуть короля в центр.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Необходимо вести пешку d в ферзи.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Ведем проходную пешку в ферзи, чтобы черные не сьели ее ладьей, используем перекрытие.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Ведем проходную пешку в ферзи, чтобы белые не сьели ее ладьей, используем перекрытие.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Ведем проходную пешку в ферзи, решает отвлечение.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Белым хотят вторгнуться королем на d6, а ладью отдать за пешку h. Тогда ладье черных не справиться с двумя проходными пешками.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Проходная пешка решает. Надо сделать зигзаг королем через поле g3, так как через f3 выигрыш не достигается: 1.Nf7 e2 2.Ne5 Ke3 3.Nd3 a4 =.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Главный враг черных далеко надвинутая проходная a, надо немедленно атаковать ее.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Прорыв - это жертва одной или нескольких пешек с целью образования проходной и проведения ее в ферзи.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных. Найдите оптимальный вариант.';
Comment2:='Проходная пешка';
Comment3:='Прорыв - это жертва одной или нескольких пешек с целью образования проходной и проведения ее в ферзи. Но не всегда он приводит к успеху.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='В ситуациях, когда у обеих сторон имеются далеко продвинутые пешки, необходимо демонстрировать изобретательность и точный расчет.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Наличие отдаленной проходной, способной отвлечь неприятельского короля от другого фланга - важный фактор оценки позиции. Здесь проходная a приносит победу.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых. Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Из двух проходных пешек чаще всего особенно опасна та, что максимально удалена от короля.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые перекрываются от шаха ладьей".'; 
Uroven:=1; 
end;
if (partNum=202) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные дают вечный шах неприятельскому королю".'; 
Uroven:=1; 
end;
if (partNum=203) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белый король прижимает черную ладью".'; 
Uroven:=1; 
end;
if (partNum=204) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые укрываются от шахов".'; 
Uroven:=1; 
end;
if (partNum=206) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черная ладья вынуждена держаться пассивно на - на 8-й горизонтали.Белые легко побеждают, переведя ладью на h7".'; 
Uroven:=1; 
end;
if (partNum=207) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные переводят ладью на вторую горизонталь и атакуют белую пешку с зади".'; 
Uroven:=1; 
end;
if (partNum=208) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные переводят ладью на первую горизонталь и атакуют белую пешку с зади".'; 
Uroven:=1; 
end;
if (partNum=209) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные не дают белой ладье встать на h8".'; 
Uroven:=1; 
end;
if (partNum=210) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые продвигают пешку на g7 и меняют ладью на f8".'; 
Uroven:=1; 
end;
if (partNum=211) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые оттесняют черного короля ладьей".'; 
Uroven:=1; 
end;
if (partNum=212) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные препятствуют переходу к позиции с пешкой на 7-й горизонтали".'; 
Uroven:=1; 
end;
if (partNum=213) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные активно защищаются".'; 
Uroven:=1; 
end;
if (partNum=214) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные приступают к шахам с длинной стороны".'; 
Uroven:=1; 
end;
if (partNum=215) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные делают ничью если их король стоит на g7".'; 
Uroven:=1; 
end;
if (partNum=216) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые прячут короля за пешку".'; 
Uroven:=1; 
end;
if (partNum=218) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые переходят к выигранной позиции ферзь против ладьи".'; 
Uroven:=1; 
end;
if (partNum=219) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые переходят к выигранной позиции с пешкой на 6-й горизонтали".'; 
Uroven:=1; 
end;
if (partNum=220) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='При пешке e5 у белого короля имелось убежище на е6 от вертикальных шахов.После того, как пешка сделала шаг вперед, королю некуда скрыться".'; 
Uroven:=1; 
end;
if (partNum=221) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Король идет на короткую сторону, оставляя длинную сторону для ладьи".'; 
Uroven:=1; 
end;
if (partNum=222) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Ладья занимает длинную сторону".'; 
Uroven:=1; 
end;
if (partNum=224) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные выманивают белую ладью".'; 
Uroven:=1; 
end;
if (partNum=225) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые переводят ладью на 1-ю линию и оттесняют черного короля".'; 
Uroven:=1; 
end;
if (partNum=226) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые готовят Kph7, неприятельская ладья не сможет потревожить короля  фланга".'; 
Uroven:=1; 
end;
if (partNum=227) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные переходят к пассивной защите с ладьей по 8-й горизонтали".'; 
Uroven:=1; 
end;
if (partNum=228) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Атака с фронта обеспечивает легкую ничью".'; 
Uroven:=1; 
end;
if (partNum=229) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные отрезают короля от пешки".'; 
Uroven:=1; 
end;
if (partNum=230) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белый король приближается к черной ладье".'; 
Uroven:=1; 
end;
if (partNum=231) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Отрезание короля слабейшей стороны по горизонтали зачастую еще более эффективно, чем по вертикали ".'; 
Uroven:=1; 
end;
if (partNum=232) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные не дают продвинуться неприятельской пешке".'; 
Uroven:=1; 
end;
if (partNum=233) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые защищают свою пешку ладьей".'; 
Uroven:=1; 
end;
if (partNum=235) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Белые создают  матывые угрозы королю противника, прижатому на краю доски".'; 
Uroven:=1; 
end;
if (partNum=236) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='При коневой пешки отрезание короля на две вертикали недостаточно для победы".'; 
Uroven:=1; 
end;
if (partNum=237) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные шахуют белого короля".'; 
Uroven:=1; 
end;
if (partNum=238) then
begin
...
Comment:= 'Ход черных.Сделайте ничью.';
Comment2:='Ладейные окончания.Ладья и пешка против ладьи.';
Comment3:='Черные подходят королем к пешке".'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Связанные пешки в такой ситуации значительно менее опасны чем разрозненные.'; 
Uroven:=1; 
end;
...
Comment:= 'Ход черных.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Черным необходимо организовать свою проходную пешку, для этого следовало сыграть h6-h5,g7-g6,f6-f5.'; 
Uroven:=1; 
end;
if (partNum=242) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Необходимо блокировать королевский фланг чтобы черные не могли там организовать проходную пешку.'; 
Uroven:=1; 
end;
if (partNum=243) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='По максимуму надвинем проходную пешку. Черному королю никак не попасть на b7, а значит использовать лишню пешку на королевском фланге черным невозможно.'; 
Uroven:=1; 
end;
if (partNum=244) then
begin
...
Comment:= 'Ход черных.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Черный король пойдет поддерживать свои проходные на ферзевый фланг. Надо попытаться не дать образовать белым проходную на ферзевом фланге, препятствовать маневру f3-f4,g3-g4,h4-h5,g4-g5'; 
Uroven:=1; 
end;
if (partNum=245) then
begin
...
Comment:= 'Ход черных.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Надо двигать пешку в проходные максимально удаленную от неприятельского короля.'; 
Uroven:=1; 
end;
if (partNum=246) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Белые разменивают свою слабую сдвоенную пешку, после чего их король успевает остаться в квадрате пешек черных.'; 
Uroven:=1; 
end;
if (partNum=247) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Конь против слона.';
Comment3:='Позицию белых стоит предпочесть, так как слон при игре на двух флангах сильнее коня. Однако можно говорить лишь о небольшом перевесе, так как пешка е5 хоть и сковывает фланг соперника, но при этом ограничивает собственного слона.'; 
Comment4:='Промедлив, белые бы получили проблемы: h5! – после этого конь переходит на f5, привязывая белых к защите слабости g3.';
Uroven:=1; 
end;
if (partNum=248) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Проходная пешка.';
Comment3:='Нужно организовать отдаленную проходную, и песочная крепость черных рухнет.'; 
Uroven:=1; 
end;
if (partNum=249) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Отличное решение - размен, предопределяющее исход поединка. На авансцене остается конь, который в большинстве случаев сильнее слона в закрытых позициях.'; 
Uroven:=1; 
end;
if (partNum=250) then
begin
...
Comment:= 'Ход белых.Найдите оптимальный вариант.';
Comment2:='Слон против коня.';
Comment3:='Нам открывается еще одно преимущество коня - его хитрость и ловкость. Белые сочетают матовые угрозы с угрозами вилок.'; 
Uroven:=1; 
end;
